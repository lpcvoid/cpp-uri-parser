...
public:
    ...
    struct URIAuthority {
        ...
        std::optional<std::string> password{}; //deprecated in rfc3986
        std::optional<uint16_t> port{};
    };
private:
    std::optional<std::string> _protocol{};
    std::optional<URIAuthority> _authority;
    std::optional<std::string> _query{};
    std::string _uri {};
    URIParsingResult _result = URIParsingResult::empty_uri;
public:
    explicit URI(std::string uri) : _uri(std::move(uri)){
        _result = parse();
    }
    ...
    [[nodiscard]] URIParsingResult get_result() const {
        return _result;
    }
...
private:
    URIParsingResult parse() {
        ...
        auto proto_term_it = std::search(_uri.begin(), _uri.end(), prot_seg_delim.begin(), prot_seg_delim.end());
        ...
        auto userinfo_term_it = std::search(_uri.begin(), _uri.end(), userinfo_delim.begin(), userinfo_delim.end());
        if (userinfo_term_it != _uri.end()) {
            std::string userinfo = std::string(proto_term_it + prot_seg_delim.size(), userinfo_term_it);
            auto userinfo_term_pass = std::search(userinfo.begin(), userinfo.end(), pass_port_delim.begin(), pass_port_delim.end());
            if (userinfo_term_pass != _uri.end()) {
                ...
                authority.password = std::string(userinfo_term_pass + pass_port_delim.size(), userinfo.end());
            } else {
                //only username
        ...
        auto slash_end_it = std::search(it_host_begin, _uri.end(), slash_delim.begin(), slash_delim.end());
        if (slash_end_it != _uri.end()) {
            //we have an end slash
            it_host_end = slash_end_it;
        } else {
            //we don't have an end slash - we can assume the uri ends here, since that's the only legal
            //character that can follow
            it_host_end = _uri.end();
        }
        //we have complete host part - check if we need to extract port too
        auto port_end_it = std::search(it_host_begin, it_host_end, pass_port_delim.begin(), pass_port_delim.end());
        if (port_end_it != _uri.end()) {
            //we have a port
            authority.host = std::string(it_host_begin, port_end_it);
            // now, either the string ends, or we have a slash
            std::string port = std::string(port_end_it + pass_port_delim.size(), it_host_end);
            if (!port.empty()) {
                // check if port is numeric
                if (std::find_if(port.begin(),port.end(), [](uint8_t c) { return !std::isdigit(c); }) == port.end()) {
                    authority.port = std::stol(port);
                }
            }
        } else {
            //only host
